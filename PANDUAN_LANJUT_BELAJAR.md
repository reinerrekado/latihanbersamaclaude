...
- **Sesi 7 (OOP) & Sesi 9 (SQL) — mulai 2026-08-31, metode BERUBAH jadi "jelasin dulu baru quiz":**
  ...
  - ✅ **Sesi 7 (OOP) — dijelasin lengkap** (class/object, `__init__`, `self`, inheritance, `super().__init__()`, `if __name__=="__main__"`, list sbg class bawaan), **tapi BELUM di-retest** — user minta jeda ("biarkan waktu bikin saya cerna", terlalu banyak info asing sekaligus). **TODO sesi berikutnya: retest OOP dulu sebelum lanjut topik baru**, jangan asumsikan udah nempel.
  - ✅ **Sesi 9 (SQL) sebagian besar — solid setelah retest:** DB vs DBMS, DDL vs DML, WHERE vs HAVING (sempat GROUP BY/HAVING KETUKER PERANNYA 2x — dikira HAVING yang "mengelompokkan", padahal itu tugas GROUP BY; ditambal pakai 3 contoh konkret berurutan — departemen/gaji, siswa/nilai, toko/penjualan — baru clean di percobaan ke-3), aggregate vs scalar function. **Subquery (WHERE/SELECT/FROM) sudah dijelasin, quiz BELUM selesai dijawab** — sesi terpotong duluan (mau lanjut dari HP via Remote Control).
  ...
  - **TODO lanjutan:** (1) retest OOP, (2) selesaikan quiz subquery, (3) lanjut ke topik JOIN (Sesi 10) yang belum di-quiz sama sekali (baru dijelasin garis besar via cheat sheet), (4) sesi 5,6,8,12 masih 100% belum disentuh sama sekali (baru ada di cheat sheet doang).
