...
- [x] Soal Medium #1 (LeetCode, Accepted) — *Group Anagrams, Accepted 2026-09-03, pakai signature `"".join(sorted(kata))` sebagai key dict + `.setdefault()`/list value*
- [ ] Submit link akun LeetCode + screenshot Accepted via Google Form *(bukti Soal Easy #1, #2, #3 sudah terkirim; Group Anagrams Accepted tapi screenshot BELUM dikirim ke form; centang penuh setelah bukti Medium terkirim)*

**🏆 CC1 SELESAI 4/4 (2026-09-03) — tinggal kirim bukti screenshot Group Anagrams ke form.**
...
- **Percobaan CC1 pertama (LeetCode "Two Sum") — 2026-08-30 malam, ~jam ke-7 belajar hari itu, BELUM selesai/submit:**
  ...
  - **Percobaan kedua & SELESAI — 2026-09-02, setelah Exam 1 lulus:** user coba brute force sendiri dulu di Jupyter, mentok, lihat pendekatan dict/complement dari Gemini sebagai referensi, lalu **tulis ulang sendiri pakai nama variabel sendiri** (`Kamus_index_angka`, `belum_masuk_kamus`) — bukan copy-paste murni, dibuktikan dari 4 bug orisinal yang muncul berurutan saat debug mandiri (titik dua `if` ketinggalan → lupa `from typing import List` di Jupyter lokal → typo nama variabel `ankga` vs `angka` → baris `print()` ketiban indentasi loop). Sempat muncul rasa "curang" karena pakai referensi Gemini — dibahas & diklarifikasi: proses lihat-referensi-lalu-bangun-ulang-dari-pemahaman itu beda dari copy-paste, dan CC1 (submission-based, bukan closed-book kayak Exam 1) memang wajar pakai referensi. **Submit ke LeetCode: Accepted, runtime 0 ms, semua 3 test case lolos.** Screenshot proof belum dikirim ke form.
...
- **CC1 (LeetCode) — mulai dikerjakan 2026-08-30 malam, progress 3/4 soal Accepted (per 2026-09-02):**
  ...
  - ✅ **"Contains Duplicate" (Easy) — ACCEPTED di LeetCode, 2026-09-02.** Percobaan ke-4 setelah 3 draft yang tangled (self-reference variabel sebelum didefinisikan, `else: return False` di dalam loop — persis jebakan `is_prime` di Sesi 5 — dan elemen Two Sum seperti `enumerate()`/dict yang ketarik masuk padahal nggak relevan). Konsep akhirnya klik lewat perbandingan skeleton eksplisit ke Two Sum ("seen set" vs "seen dict") dan kode dikasih langsung sebagai kerangka lengkap (bukan cuma dibimbing trace) karena 3x percobaan mandiri berturut-turut belum konvergen. Pola solusi: `set()` kosong sebagai "sudah pernah dilihat", `return True` begitu ketemu, `return False` di LUAR loop (rata kiri `for`). **Bukti screenshot SUDAH dikirim ke form** (2026-09-02).
  - ✅ **"Group Anagrams" (Medium) — ACCEPTED di LeetCode, 2026-09-03.** Sempat dibandingkan dulu sama "Longest Substring Without Repeating Characters" (sliding window, dinilai lebih berat) dan "Combination Sum" (backtracking/rekursi, jauh lebih berat) — Group Anagrams dipilih karena masih reuse pola dict yang sudah dikuasai. Insight kunci yang ditemukan sendiri: `sorted(list_kata)` (urutkan daftar kata) ≠ `sorted(satu_kata)` (urutkan huruf dalam 1 kata) — baru nemu bedanya lewat trial langsung di Jupyter. Sempat 3 draft tangled: (1) ke-influence pola "return begitu ketemu match pertama" dari Two Sum padahal soal ini butuh proses SEMUA kata dulu baru output, (2) lupa `return` sama sekali (function balikin `None`/"null" — trap klasik return vs print, Sesi 3), (3) sempat submit ke LeetCode duluan sebelum dites lokal sampai kena rate-limit "submit too frequently". Pola solusi akhir: `signature = "".join(sorted(kata))` sebagai key dict, `.setdefault(signature, []).append(kata)` buat grouping, `return list(kamus.values())` di LUAR loop.
  - **CC1 TUNTAS 4/4 (2026-09-03).** Sisa: kirim screenshot bukti Group Anagrams ke form.
